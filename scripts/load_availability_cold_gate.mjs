...
function buildUrl(i) {
  const date = addDays(BASE_DATE, i); // unique date => cold path
...
async function oneRequest(i) {
  ...
  try {
    ...
    const ms = performance.now() - started;
    return { ok: false, status: 'FETCH_ERR', ms };
  }
}
...
async function main() {
    ...
    JSON.stringify(
      {
        ...
        BASE_DATE,
        uniqueDates: TOTAL,
        LOAD_BYPASS_TOKEN: LOAD_BYPASS_TOKEN ? 'set' : '',
      },
      null,
      2,
    ),
  ...
  console.log(`STATUSES=${JSON.stringify(Object.fromEntries(Object.entries(statuses).sort((a, b) => String(a[0]).localeCompare(String(b[0])))), null, 2)}`);
