...
: > /tmp/plantak_api.log
nohup bash -lc "cd \"$APP_DIR\" && set -a && [ -f .env ] && . ./.env && set +a && exec env PORT=${PORT} THROTTLE_BYPASS_TOKEN=${THROTTLE_BYPASS_TOKEN:-} node dist/main" >/tmp/plantak_api.log 2>&1 < /dev/null &
sleep 6
