...
async function main() {
  ...
  const key = `deposit-capture-proof-${Date.now()}`;
  ...
  assert(
    paid?.paymentStatus === 'REMAINING_DUE_IN_SALON' || paid?.paymentStatus === 'PAID',
    `UNEXPECTED_PAYMENT_STATUS_${paid?.paymentStatus}`,
  );
  ...
  assert(txs.length === 1, `DEPOSIT_CAPTURE_TX_COUNT_${txs.length}`);
  assert(
    txs[0].amountCents === (dbBooking.amountDepositCentsSnapshot ?? 0),
    `DEPOSIT_CAPTURE_AMOUNT_MISMATCH_${txs[0].amountCents}_EXPECTED_${dbBooking.amountDepositCentsSnapshot ?? 0}`,
  );
  ...
  console.log('PAYMENT_DEPOSIT_CAPTURE_PROOF_OK');
}
