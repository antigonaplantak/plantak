...
section "OPENING_BOUNDARY_PROOF"
...
# Monday 09:00-10:00, service totalMin=50, interval=30
curl -fsS -X PUT "$API/staff/$STAFF_ID/working-hours" \
  -H "authorization: Bearer $TOKEN" \
  -H 'content-type: application/json' \
  -d "{\"businessId\":\"$BUSINESS_ID\",\"items\":[{\"dayOfWeek\":1,\"startMin\":540,\"endMin\":600}]}" >/dev/null
...
expected = ["2026-03-30 09:00 +0200"]

print("ACTUAL=", actual)
if actual != expected:
    raise SystemExit("OPENING_BOUNDARY_NOT_GREEN")

print("OPENING_BOUNDARY_PROOF_OK")
PY
