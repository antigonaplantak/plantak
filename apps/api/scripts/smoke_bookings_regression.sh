...
read DATE_YMD START_AT NEW_START_AT <<EOF
$(python3 - <<'PY'
from datetime import datetime, timedelta, timezone
now = datetime.now(timezone.utc) + timedelta(days=60)
hour = 10 + (now.second % 6)
minute = ((now.minute // 5) * 5)
start = now.replace(hour=hour, minute=minute, second=0, microsecond=0)
new = start + timedelta(hours=2)
print(start.strftime("%Y-%m-%d"), start.strftime("%Y-%m-%dT%H:%M:%S.000Z"), new.strftime("%Y-%m-%dT%H:%M:%S.000Z"))
PY
)
EOF
...
echo "== AUTH =="
REQ="$(curl -sS -X POST "$API/auth/magic/request" -H "Content-Type: application/json" -d "{\"email\":\"$OWNER_EMAIL\"}")"
CODE="$(printf "%s" "$REQ" | python3 -c "import sys,json; print(json.load(sys.stdin).get(\"devCode\",\"\"))")"
VER="$(curl -sS -X POST "$API/auth/magic/verify" -H "Content-Type: application/json" -d "{\"email\":\"$OWNER_EMAIL\",\"code\":\"$CODE\"}")"
TOKEN="$(printf "%s" "$VER" | python3 -c "import sys,json; print(json.load(sys.stdin).get(\"accessToken\",\"\"))")"
...
CREATE_RES="$(curl -sS -w "\nHTTP=%{http_code}\n" -X POST "$API/bookings" \
  ...
  -d "{\"businessId\":\"$BUSINESS_ID\",\"staffId\":\"$STAFF_ID\",\"serviceId\":\"$SERVICE_ID\",\"startAt\":\"$START_AT\",\"tz\":\"$TZ\",\"idempotencyKey\":\"smoke-create-$(date +%s)\"}")"
echo "$CREATE_RES"
echo

CREATE_HTTP="$(printf "%s" "$CREATE_RES" | sed -n "s/^HTTP=//p")"
[ "$CREATE_HTTP" = "201" ] || { echo "CREATE_FAILED"; exit 1; }
...
[ "$CONFIRM_HTTP" = "201" ] || { echo "CONFIRM_FAILED"; exit 1; }
...
RESCHEDULE_RES="$(curl -sS -w "\nHTTP=%{http_code}\n" -X POST "$API/bookings/$BOOKING_ID/reschedule" \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
  -d "{\"businessId\":\"$BUSINESS_ID\",\"newStartAt\":\"$NEW_START_AT\",\"tz\":\"$TZ\",\"idempotencyKey\":\"smoke-reschedule-$(date +%s)\"}")"
echo "$RESCHEDULE_RES"
echo
