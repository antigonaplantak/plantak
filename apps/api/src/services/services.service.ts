...
export class ServicesService {
  ...
  async updateCategory(userId: string, id: string, dto: UpdateServiceCategoryDto) {
    const existing = await this.prisma.serviceCategory.findUnique({ where: { id } });
    if (!existing || existing.archivedAt) throw new NotFoundException("Category not found");
    await this.assertBusinessAccess(userId, existing.businessId, ["OWNER", "ADMIN"]);
  ...
  async listAdminServices(userId: string, businessId: string) {
    ...
    return this.prisma.service.findMany({
      ...
      include: {
        category: true,
        variants: { where: { archivedAt: null }, orderBy: [{ position: "asc" }, { createdAt: "asc" }] },
        addons: { where: { archivedAt: null }, orderBy: [{ position: "asc" }, { createdAt: "asc" }] },
      },
      orderBy: [{ isPinned: "desc" }, { position: "asc" }, { createdAt: "asc" }],
    });
  }
  ...
  async getService(userId: string, serviceId: string) {
    ...
    const [category, variants, addons, staffAssignments] = await Promise.all([
      base.categoryId
        ? this.prisma.serviceCategory.findUnique({ where: { id: base.categoryId } })
  ...
  async setServiceStatus(userId: string, serviceId: string, dto: SetServiceStatusDto) {
    const existing = await this.getServiceOrThrow(serviceId);
    await this.assertBusinessAccess(userId, existing.businessId, ["OWNER", "ADMIN"]);
    const updated = await this.prisma.service.update({
      where: { id: serviceId },
  ...
  async replaceStaffServices(userId: string, staffId: string, dto: ReplaceStaffServicesDto) {
    await this.assertBusinessAccess(userId, dto.businessId, ["OWNER", "ADMIN"]);
